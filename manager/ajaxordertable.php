...
$increment = 1;
try {
    $statementgetitems = $conn->prepare("SELECT * FROM items WHERE orderid = ?");
...
?>
        <tr>
            ...
            <td>
                <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#edititemmodal" id="<?php echo $row[0]; ?>" onclick="getitemdatatomodal(this.id, <?php echo $orderid ?>)">
            ...
            <td><?php
                ...
                ?>
                <span class="badge rounded-pill bg-<?php echo $badgecolor ?>"><?php echo $statustext ?></span><br>
                <?php
                $progress = $row[6];
                echo ($progress/1)*100; ?>%
            </td>
